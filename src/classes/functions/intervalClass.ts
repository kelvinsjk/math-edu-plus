...
export default class Interval {
  ...
  contains(x: number | Fraction): boolean {
    ...
    const leftOfUpperBound =
      this.right.valueOf() > x.valueOf() || (this.right.valueOf() === x.valueOf() && !this.rOpen);
  ...
  intersect(interval2: Interval): Interval | null {
    ...
    if (this.left.valueOf() === interval2.left.valueOf()) {
      ...
      if (thisIsSmaller) {
        // take interval2
        left =
          interval2.left instanceof Fraction ? new Fraction(interval2.left.num, interval2.left.den) : interval2.left;
    ...
    if (this.right.valueOf() === interval2.right.valueOf()) {
      ...
      if (thisIsLarger) {
        // take interval2
        right =
          interval2.right instanceof Fraction
            ? new Fraction(interval2.right.num, interval2.right.den)
            : interval2.right;
  ...
  union(interval2: Interval): Interval[] {
    ...
    if (this.intersect(interval2) === null) {
      ...
      // if a) \cup [a, can join
      const canJoinCase1 =
        thisIsSmaller && this.right.valueOf() === interval2.left.valueOf() && (!this.rOpen || !interval2.lOpen);
      const canJoinCase2 =
        !thisIsSmaller && this.left.valueOf() === interval2.right.valueOf() && (!this.lOpen || !interval2.rOpen);
  ...
  times(x: number | Fraction): Interval {
    let left: number | Fraction, right: number | Fraction;
    if (x instanceof Fraction) {
      left = this.left === -Infinity ? (x.valueOf() < 0 ? Infinity : -Infinity) : x.times(this.left);
      right = this.right === Infinity ? (x.valueOf() < 0 ? -Infinity : Infinity) : x.times(this.right);
    } else {
      left = typeof this.left === 'number' ? x * this.left : this.left.times(x);
      right = typeof this.right === 'number' ? x * this.right : this.right.times(x);
    }
    return x.valueOf() < 0
      ? new Interval(right, left, this.rOpen, this.lOpen)
      : new Interval(left, right, this.lOpen, this.rOpen);
  }
  /**
   * exponentiation by `n`
   */
      ...
      if (this.left.valueOf() === 0) {
        ...
        left =
          typeof this.left === 'number'
            ? this.left === -Infinity
              ? 0
              : Fraction.ONE.divide(Math.pow(this.left, Math.abs(n)))
      ...
      if (this.right.valueOf() === 0) {
        ...
        right =
          typeof this.right === 'number'
            ? this.right === Infinity
              ? 0
              : Fraction.ONE.divide(Math.pow(this.right, Math.abs(n)))
