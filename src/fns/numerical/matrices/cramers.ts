...
export default function cramers(...args: number[]): number[] {
  if (args.length === 6) {
    ...
    const detX = determinant(args[3], ...args.slice(1, 3), args[7], ...args.slice(5, 7), args[11], ...args.slice(9, 11));
    const detY = determinant(args[0], args[3], args[2], args[4], args[7], args[6], args[8], args[11], args[10]);
    const detZ = determinant(...args.slice(0, 2), args[3], ...args.slice(4, 6), args[7], ...args.slice(8, 10), args[11]);
