...
import { Fraction, Expression, Angle } from 'math-edu';
// '../../../../math-edu/src/index';
  ...
  if (!(u.n.isInteger() && u.n.valueOf() >= 0)) {
    ...
    if (u.n.isEqual(0)) {
      ...
      if (limits === undefined) {
        ...
        const vTerm1 =
          v instanceof ExpFn ? v.valueAt(limits.upper as number | Fraction) : v.valueAt(limits.upper as number | Angle);
        const vTerm2 =
          v instanceof ExpFn ? v.valueAt(limits.lower as number | Fraction) : v.valueAt(limits.lower as number | Angle);
