...
function binomCdf(n: number, p: number, x: number): number{
  x = Math.floor(x);
  return binomialDistribution(n,p).slice(0,x+1).reduce((x,y)=>x+y);
}
...
function binomCdfRange(n: number, p: number, lower: number, upper: number): number{
  ...
  const p2 = binomialDistribution(n,p).slice(0,upper+1).reduce((x,y)=>x+y);
  const p1 = binomialDistribution(n, p).slice(0, lower).reduce((x, y) => x + y);
  return p2 - p1;
}
