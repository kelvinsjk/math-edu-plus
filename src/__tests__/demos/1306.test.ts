...
test('1306', () => {
  ...
  expect(q1[7]).toBe("(Hint: you may want to consider expressing 2 \\mathrm{i} in the form 2 \\mathrm{e}^{\\mathrm{i} \\alpha}");
  ...
  for (let i = 0; i < 30; i++){
    const { questions: [q], qnCode: qnCode } = qn1306();
    const { questions: [q2] } = qn1306({ qnCode: qnCode });
    expect(q).toBe(q2);
  }
