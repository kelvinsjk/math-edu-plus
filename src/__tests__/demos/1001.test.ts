...
const { questions: [q1, q1Y, q1X, q1Y0, q1X0], answers: [a1a, a1b], qnCode: qc1 } = qn1001({ qnCode: qCode1 });
const { questions: [q2, q2Y, q2X, q2Y0, q2X0], answers: [a2a, a2b], qnCode: qc2 } = qn1001({ qnCode: '1b859' });
...
test('1001', () => {
  ...
  for (let i = 0; i < 30; i++){
    const { questions: [q], qnCode: qnCode } = qn1001();
    const { questions: [q2] } = qn1001({ qnCode: qnCode });
    expect(q).toBe(q2);
  }
