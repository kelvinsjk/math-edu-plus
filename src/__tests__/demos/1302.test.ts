...
const { questions: [q1], answers: [a1], qnCode: qc1 } = qn1302({ qnCode: qCode1 });
const { questions: [q2], answers: [a2] } = qn1302({ qnCode: '2472' });
const { questions: [q3], answers: [a3] } = qn1302({ qnCode: '3EA1' });
const { questions: [q4], answers: [a4] } = qn1302({ qnCode: '4HJ4' });
...
test('1302', () => {
  ...
  for (let i = 0; i < 30; i++){
    const { questions: [q], qnCode: qnCode } = qn1302();
    const { questions: [q2] } = qn1302({ qnCode: qnCode });
    expect(q).toBe(q2);
  }
