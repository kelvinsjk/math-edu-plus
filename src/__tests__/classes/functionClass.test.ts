...
test('basic function constructor', () => {
  ...
  expect(
    () => new BasicFunction('x+a', twoThird, { domain: [new Interval(-Infinity, -2), new Interval(3)] }),
  ).not.toThrow();
  expect(
    () => new BasicFunction('ax', twoThird, { domain: [new Interval(-Infinity, -2), new Interval(3)] }),
  ).not.toThrow();
