...
//} from 'math-edu';
import {
  Term, Polynomial, getRandomInt, factorize, Fraction, Expression
...
function variablesToQn(type: number, subtype: string, restOfVariables: number[]): [variablesObject1a|variablesObject1b, string[], string[]] {
  /// if (type === 1){
    if (subtype === 'a') {
      const a = restOfVariables[0], b = restOfVariables[1], k = restOfVariables[2], A = restOfVariables[3];
      ...
      const a = restOfVariables[0], b = restOfVariables[1], c = restOfVariables[2];
...
function type1aGenerator(variables?: variablesObject1a): [variablesObject1a, string[], string[]] {
  ...
  const qnVariable1 = 'y', qnVariable2 = 'x';
  // initial conditions
  const initialY = new Fraction(a - A, b);
  const qnInitial1 = `y = ${initialY}`, qnInitial2 = 'x = 0';
  // answer
  ...
  // return
  return [variables,
    [qn, qnVariable1, qnVariable2, qnInitial1, qnInitial2],
    [ansA, ansB]
  ];
}
...
function type1bGenerator(variables?: variablesObject1b): [variablesObject1b, string[], string[]] {
  ...
  const qnVariable1 = 'x', qnVariable2 = 'y';
  // initial conditions
  const initialX = new Fraction(c - 1, a * b);
  const qnInitial1 = `x = ${initialX}`, qnInitial2 = 'y = 0';
  // answer
