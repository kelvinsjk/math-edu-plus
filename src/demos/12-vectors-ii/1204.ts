...
//} from 'math-edu';
import { getRandomInt, getRandomVec, getRandomFrac, Vector, Fraction, Line, Plane } from 'math-edu';
// '../../../../math-edu/src/index';
...
function typeAGenerator(variables?: variablesObjectA): [variablesObjectA, string[], string[]] {
  ...
  if (variables === undefined) {
    ...
    variables = {
      ...
      ny: n.y.num,
      nz: n.z.num,
      lambdaNum: lambda.num,
      lambdaDen: lambda.den,
      type: 1,
    };
  } else {
  ...
  // store qns, answers
  const qns = [aPoint, bPoint, planeEquation, distanceQn];
  const answers = [intersection.toCoordinates(), angle, distanceString];
  // return
...
function typeBGenerator(variables?: variablesObjectB): [variablesObjectB, string[], string[]] {
  ...
  // store qns, answers
  const qns = [aPoint, dp1String, dp2String, lineEquation, distanceQn];
  const answers = [intersection.toCoordinates(), angle, distanceString];
  // return
  return [variables, qns, answers];
}
