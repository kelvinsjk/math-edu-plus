...
//} from 'math-edu';
import {
  getRandomInt, getRandomVec, factorize, Vector, Fraction
} from
  'math-edu';
//  '../../../../math-edu/src/index';
...
function qn1103(options?: qnOptions): qnOutput {
  ...
  if (optionsObject.qnCode) { // qnCode provided
    ...
    let c = getRandomVec();
    while (c.isParallelTo(a) || c.isParallelTo(b) || getRandomInt(0,10) === 0) { // last condition to trigger test coverage
      c = getRandomVec();
    }
    let [lambda, mu] = getRandomInts(2, { unique: false, min: 1, max: 5 });
    [[lambda, mu]] = factorize(lambda, mu);
    const restOfVariables = [a.x.num, a.y.num, a.z.num, b.x.num, b.y.num, b.z.num, c.x.num, c.y.num, c.z.num, lambda, mu];
    [variables, questions, answers] = variablesToQn(type1, type2, type3, restOfVariables);
  }
  // prepare variablesArray for encoding
  const variablesArray = [variables.type[0], variables.type[1], variables.type[2], variables.ax, variables.ay, variables.az, variables.bx, variables.by, variables.bz, variables.cx, variables.cy, variables.cz, variables.lambda, variables.mu];
  return {
    variables: variables,
...
function variablesToQn(type1: string, type2: string, type3: string, restOfVariables: number[]): [variablesObject, string[], string[]] {
  ...
  const lambda = restOfVariables[9], mu = restOfVariables[10];
  const variablesObject = {
    ax: restOfVariables[0], ay: restOfVariables[1], az: restOfVariables[2],
    bx: restOfVariables[3], by: restOfVariables[4], bz: restOfVariables[5],
    cx: restOfVariables[6], cy: restOfVariables[7], cz: restOfVariables[8],
    lambda: lambda, mu: mu, type: type1+type2+type3
  };
  // generate question
  let ratio: string;
  if (type1 === 'a') { // V is on AB
    ratio = type2 === 'a' ? `AV:VB = ${lambda}:${mu}` : `AV:AB = ${lambda}:${lambda+mu}`;
  } else { // V is on AB extended
    ratio = type2 === 'a' ? `AB:BV = ${lambda}:${mu}` : `AB:AV = ${lambda}:${lambda + mu}`;
  }
  const triangle = type3 === 'a' ? 'OAC' : 'OBC';
  const qns = [a.toString({ ijkMode: true }), b.toString({ ijkMode: true }), ratio, c.toString({ ijkMode: true }), triangle];
  // generate answers
  const v = type1 === 'a' ? Vector.ratioTheorem(a, b, lambda, mu) : Vector.ratioTheorem(a, b, lambda, mu, { extendedMode: true });
  v.simplify();
