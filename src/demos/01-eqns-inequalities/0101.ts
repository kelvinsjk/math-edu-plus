...
function qn0101(options?: qnOptions): qnOutput {
  ...
  if (optionsObject.qnCode) {
    ...
    if (type > 7) {
      // more chances to get type 1 and 2
      type = type % 6;
    }
    // generate sign
    const signs = ['<', '>'];
  ...
  if (type === 1) {
    ...
    variablesArray = [
      ...
      variables.root1Num,
      variables.root1Den,
      (<variablesObjectCD>variables).root2Num,
      (<variablesObjectCD>variables).root2Den,
      variables.rhs,
      (<variablesObjectCD>variables).numVsDen,
      (<variablesObjectCD>variables).negative,
    ];
  } else if (type === 5) {
...
): [variablesObjectA | variablesObjectB | variablesObjectCD | variablesObjectE | variablesObjectF, string[], string[]] {
...
function typeAGenerator(sign: string, variables?: variablesObjectA): [variablesObjectA, string[], string[]] {
  ...
  if (variables === undefined) {
    ...
    root3Num = roots[0].num;
    root3Den = roots[0].den;
    root4Num = roots[1].num;
    root4Den = roots[1].den;
    ...
    const root3 = new Fraction(root3Num, root3Den),
      root4 = new Fraction(root4Num, root4Den);
    roots = [root3, root4];
    quadratic1 = Polynomial.fromRoots(root1Int, root2Int);
...
function typeCDGenerator(
  sign: string,
  type: number,
  variables?: variablesObjectCD,
): [variablesObjectCD, string[], string[]] {
  let a: number,
    b: number,
    c: number,
    root1Num: number,
  ...
  if (variables === undefined) {
    ...
    root1Num = roots[0].num;
    root1Den = roots[0].den;
    root2Num = roots[1].num;
    root2Den = roots[1].den;
    ({
      quadratic: quadratic2,
      coefficients: [a, b, c],
    } = getRandomQuadratic({ monic: true, complex: true }));
    ...
    const root3 = new Fraction(root1Num, root1Den),
      root4 = new Fraction(root2Num, root2Den);
    roots = [root3, root4];
