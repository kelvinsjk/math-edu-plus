...
function qn0102(options?: {qnCode?: string, position?: number}): qnOutput {
  ...
  const variablesArray = [variables.ax, variables.ay, variables.az, variables.bx, variables.by, variables.bz, variables.cx, variables.cy, variables.cz, variables.dx, variables.dy, variables.dz, variables.x, variables.y, variables.z];
  return {
    variables: variables,
...
function generateVariables(position: number): VariablesObject{
  const ax = numberFromRandom(getRandomInt(1, 89)), ay = numberFromRandom(getRandomInt(1, 89)), az = numberFromRandom(getRandomInt(1, 89));
  const aVec = new Vector(ax, ay, az);
  let bx = numberFromRandom(getRandomInt(1, 89)), by = numberFromRandom(getRandomInt(1, 89)), bz = numberFromRandom(getRandomInt(1, 89));
  let bVec = new Vector(bx, by, bz);
  while (aVec.isParallelTo(bVec)
    || getRandomInt(0,9) === 0 // to facilitate testing
    ) {
    bx = numberFromRandom(getRandomInt(1, 89)); by = numberFromRandom(getRandomInt(1, 89)); bz = numberFromRandom(getRandomInt(1, 89));
    bVec = new Vector(bx, by, bz);
  }
  const n = aVec.cross(bVec);
  let cx = numberFromRandom(getRandomInt(1, 89)), cy = numberFromRandom(getRandomInt(1, 89)), cz = numberFromRandom(getRandomInt(1, 89));
  let cVec = new Vector(cx, cy, cz);
  while (cVec.dot(n).isEqual(0)
    || getRandomInt(0, 9) === 0 // to facilitate testing
    ) {
    cx = numberFromRandom(getRandomInt(1, 89)); cy = numberFromRandom(getRandomInt(1, 89)); cz = numberFromRandom(getRandomInt(1, 89));
    cVec = new Vector(cx, cy, cz);
  }
  let dx = numberFromRandom(getRandomInt(1, 89)), dy = numberFromRandom(getRandomInt(1, 89)), dz = numberFromRandom(getRandomInt(1, 89));
  let dVec = new Vector(dx, dy, dz);
  while (aVec.isParallelTo(dVec) || bVec.isParallelTo(dVec) || cVec.isParallelTo(dVec)
    || getRandomInt(0, 9) === 0 // to facilitate testing
    ) {
    dx = numberFromRandom(getRandomInt(1, 89)); dy = numberFromRandom(getRandomInt(1, 89)); dz = numberFromRandom(getRandomInt(1, 89));
    dVec = new Vector(dx, dy, dz);
  }
  const x = numberFromRandom(getRandomInt(1, 89)), y = numberFromRandom(getRandomInt(1, 89)), z = numberFromRandom(getRandomInt(1, 89));
  const ta = ax * x + ay * y + az * z, tb = bx * x + by * y + bz * z, tc = cx * x + cy * y + cz * z, td = dx * x + dy * y + dz * z;

  if ((ta % 100 !== 0 || tb % 100 !== 0 || tc % 100 !== 0 || td % 100 != 0)) {
    return generateVariables(position);
  }
  return {
    ax, ay, az, bx, by, bz, cx, cy, cz, dx, dy, dz, x, y, z,
    position
  }    
}