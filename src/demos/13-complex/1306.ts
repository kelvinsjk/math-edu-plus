...
//} from 'math-edu';
import {
  getRandomInt, Angle, Trig, Expression, Complex
...
export function qn1306(options?: qnOptions): qnOutput {
  ...
  if (type === 1) {
    ...
    variablesArray =
      [variables.type, variables.R, variables.m, variables.n, variables.secondSign];
  } else { // type 2
    variables = variables as variablesObjectB;
    variablesArray =
      [variables.type, variables.x1, variables.y1, variables.x2, variables.y2];
  }
  return {
    variables: variables,
...
function typeAGenerator(variables?: variablesObjectA): [variablesObjectA, string[], string[]] {
  ...
  const hint = n === 6 ? `(Hint: you may want to consider expressing ${R} \\mathrm{i} in the form ${R} \\mathrm{e}^{\\mathrm{i} \\alpha}` : '';
  ...
  // store qns, answers
  const qns = [
    w, wShow, secondRoot, thirdRoot, equation, expression, numberOfFactors, hint
  ]
  const answers = [
    kEquals, `${quadraticOne}`, `${quadraticTwo}`, quadraticThree
  ]
  // return
...
function typeBGenerator(variables?: variablesObjectB): [variablesObjectB, string[], string[]] {
  ...
  if (variables === undefined) {
    ...
    while ((Math.abs(x1) === Math.abs(x2) && Math.abs(y1) === Math.abs(y2))
  ...
  // store qns/answers
  const qns = [
    polynomialQn, `${root1}`, `${root2}`
  ];
  const answers = [
    `P(z) = ( ${quadraticOne} )( ${quadraticTwo} )`,
    `b = ${b}`,
    `c = ${c}`,
    `d = ${d}`
  ]
